...
export default function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0D0D0D] text-white flex items-center justify-center p-4 sm:p-8 relative overflow-hidden">
      
      {/* Cinematic Background Engine (Video + Vignette + Refraction) */}
      <CinematicBackground />
      ...
      <motion.div 
        layout
        className="glass-platinum w-full max-w-3xl relative overflow-hidden p-8 sm:p-12 rounded-3xl z-20 border border-white/20 shadow-2xl"
      >
        <AnimatePresence mode="wait">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      <button 
                        ...
                        className={`py-3.5 px-4 rounded-xl font-mono text-xs uppercase tracking-wider font-bold transition-all border ${formData.vertical === v.id ? 'bg-[#5EC8F2] text-[#0D0D0D] border-[#5EC8F2] shadow-[0_0_25px_rgba(94,200,242,0.4)]' : 'bg-black/30 text-white border-white/15 hover:border-white/40 hover:bg-white/10'}`}
                      >
                        {v.label}
                      </button>
                    ))}
          ...
          {step === 2 && (
            ...
            >
              ...
              <p className="text-slate-200 text-sm sm:text-base font-sans leading-relaxed">
                Resumen ejecutivo y carga de documentación. El orquestador estructurará el Marco Lógico y los indicadores RACER.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-2 border-dashed border-white/25 hover:border-[#5EC8F2] rounded-2xl p-6 flex flex-col items-center justify-center text-center bg-black/30 hover:bg-white/10 transition-all relative group cursor-pointer">
                    ...
                    <p className="text-slate-300 text-[10px] font-mono mt-1">PDF, TXT, DOCX (Máx. 50MB)</p>
                  </div>
                  
                  {documents.length > 0 && (
                    <div className="space-y-2 mt-3">
                      <h4 className="text-[10px] font-mono uppercase tracking-wider text-slate-300 font-bold">Contexto Cargado:</h4>
                      {documents.map((d, i) => (
                        <div key={i} className="flex items-center gap-2 bg-black/50 border border-white/15 p-2.5 rounded-xl text-xs text-white">
              ...
              <div className="bg-black/50 border border-white/15 rounded-2xl p-4">
                <div className="flex justify-between items-center mb-2">
                  <h4 className="text-[10px] font-mono uppercase tracking-wider text-slate-300 font-bold">Estimación de Ventana de Contexto</h4>
              ...
              <div className="flex justify-between items-center mt-2">
                <button onClick={() => setStep(1)} className="text-slate-300 hover:text-white font-mono text-xs uppercase tracking-wider font-bold">
                  ← Atrás
                </button>
                <NeuralButton 
                  onClick={() => setStep(3)}
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button 
                  ...
                  className={`p-6 text-left rounded-2xl transition-all border ${formData.model === 'claude-3-haiku-20240307' ? 'bg-[#5EC8F2]/20 border-[#5EC8F2] shadow-[0_0_30px_rgba(94,200,242,0.3)]' : 'bg-black/30 border-white/15 hover:bg-white/10'}`}
                >
                  ...
                  <p className="text-slate-300 text-xs font-sans leading-relaxed">Procesamiento ultra-rápido para mapeo lógico directo e extracción rápida de indicadores.</p>
                </button>

                <button 
                  onClick={() => setFormData({ ...formData, model: 'claude-3-5-sonnet-20240620' })}
                  className={`p-6 text-left rounded-2xl transition-all border ${formData.model === 'claude-3-5-sonnet-20240620' ? 'bg-[#5EC8F2]/20 border-[#5EC8F2] shadow-[0_0_30px_rgba(94,200,242,0.3)]' : 'bg-black/30 border-white/15 hover:bg-white/10'}`}
                >
                  <h3 className="text-lg font-syne font-bold text-white mb-2 flex items-center justify-between">
                    Claude 3.5 Sonnet
                    {formData.model === 'claude-3-5-sonnet-20240620' && <CheckCircle2 className="w-5 h-5 text-[#5EC8F2]" />}
                  </h3>
                  <p className="text-slate-300 text-xs font-sans leading-relaxed">Análisis profundo y matizado para activos complejos con documentación extensa.</p>
                </button>
              </div>

              <div className="flex justify-between items-center mt-6">
                <button onClick={() => setStep(2)} className="text-slate-300 hover:text-white font-mono text-xs uppercase tracking-wider font-bold">
                  ← Atrás
                </button>
                <NeuralButton 
