  ...
  return (
    <div className="min-h-screen p-6 md:p-12 bg-[#1A1A2E] text-white relative overflow-hidden font-sans">
      ...
      <div className="max-w-6xl mx-auto relative z-20">
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              <span className="px-3 py-1 bg-white/5 text-slate-300 border border-white/10 rounded-xl text-[10px] uppercase font-mono tracking-widest flex items-center gap-1.5 font-bold">
                <ShieldCheck className="w-3.5 h-3.5 text-slate-400" /> INDEPENDENT QUALITY AUDITOR PORTAL
              </span>
            </div>
            <h1 className="text-3xl md:text-5xl font-syne font-bold text-white tracking-tight">Milestone Audit & Payout Approval</h1>
        ...
        {/* Success Notification - Neutralized for No Green Rule */}
        {successMessage && (
          <motion.div
        ...
        <div className="space-y-8">
          <h2 className="text-2xl font-syne font-bold text-white flex items-center gap-2">
            <Award className="w-5 h-5 text-slate-400" /> Pending Milestone Verifications
          </h2>
          ...
          {loading ? (
            <div className="p-16 text-center text-slate-400 bg-black/20 border border-white/5 rounded-xl backdrop-blur-xl">
              ...
              <p className="text-[13px] font-mono uppercase tracking-widest">Loading auditor verification queue...</p>
            </div>
          ) : reports.length === 0 ? (
                ...
                >
                  <div className="space-y-4 max-w-2xl">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[12px] pt-2">
                      <div className="p-4 bg-white/5 rounded-xl border border-white/5">
                        <span className="text-slate-500 block text-[9px] font-mono uppercase tracking-widest font-bold mb-2">Performance Metric</span>
                        <span className="text-slate-200 font-sans font-medium">{report.indicatorName}</span>
                      </div>
                      <div className="p-4 bg-white/5 rounded-xl border border-white/5">
                        <span className="text-slate-500 block text-[9px] font-mono uppercase tracking-widest font-bold mb-2">Verification Source</span>
                  ...
                  <div className="flex flex-col justify-between items-end border-t md:border-t-0 md:border-l border-white/5 pt-6 md:pt-0 md:pl-8">
                    <div className="text-right mb-6 md:mb-0">
                      <span className="text-[10px] text-slate-500 font-mono uppercase tracking-widest font-bold block mb-2">Tranche Amount</span>
                    ...
                    {report.status === 'APPROVED' ? (
                      <div className="px-5 py-2.5 bg-[#5EC8F2]/10 text-white border border-[#5EC8F2]/30 rounded-xl text-[12px] font-mono font-bold uppercase tracking-wider flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-[#5EC8F2]" /> Released
                      </div>
                    ) : (
                      <button
                      ...
                      >
                        {approvingId === report.id ? (
                          ...
                          <>
                            <ShieldCheck className="w-4 h-4 text-slate-300" /> Approve & Release
                          </>
                        )}
                      </button>
                    )}
