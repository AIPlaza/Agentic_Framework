...
// Generator helper to produce 126 structured PM stories
function generateInitialStories(projectTitle: string): TaskItem[] {
  ...
  const baseTemplates = [
    ...
    { title: 'Implement Daily Operational Quality Log Schema (BDO ISO 9001)', cat: 'Quality & ISO 9001', status: 'DESIGN', p: 'HIGH' },
  ...
  // Generate 126 tasks programmatically
  for (let i = 1; i <= 126; i++) {
    ...
    stories.push({
      ...
      title: `${base.title} [Batch #${Math.ceil(i / 10)}]`,
      category: cat,
      priority,
      status,
      assignee,
...
export function KanbanBoard({ projectTitle = 'Clean Biogas Facility' }: { projectTitle?: string }) {
  ...
  const columns: { id: 'BACKLOG' | 'DESIGN' | 'EXECUTION' | 'AUDIT' | 'DONE'; label: string; color: string; badgeBg: string }[] = [
    { id: 'BACKLOG', label: '1. Backlog & Requirements', color: 'border-slate-500/30', badgeBg: 'bg-slate-500/20 text-slate-300' },
  ...
  const moveTaskStatus = (id: string, newStatus: 'BACKLOG' | 'DESIGN' | 'EXECUTION' | 'AUDIT' | 'DONE') => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-[#020624]/60 p-4 rounded-2xl border border-white/10 backdrop-blur-xl">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full bg-black/40 border border-white/15 rounded-xl py-2 pl-10 pr-4 text-xs font-mono text-white placeholder-slate-400 focus:outline-none focus:border-[#5EC8F2] transition-all"
          />
        </div>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-[11px] font-bold transition-all ${
                selectedCategory === cat
                  ? 'bg-gradient-to-r from-[#5EC8F2] to-[#377D8C] text-[#020624]'
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 items-start">
        {columns.map((col) => {
          ...
          return (
            <div
              key={col.id}
              className={`p-4 bg-[#020624]/40 border rounded-2xl space-y-3 backdrop-blur-xl min-h-[500px] ${col.color}`}
            >
              {/* Column Header */}
              ...
              <div className="space-y-3">
                {colTasks.slice(0, 15).map((task) => (
                  <motion.div
                    ...
                    className="p-3.5 bg-black/50 border border-white/10 hover:border-[#5EC8F2]/50 rounded-xl space-y-2.5 cursor-pointer hover:shadow-lg transition-all group relative overflow-hidden"
                  >
                    {/* Top Decorator */}
                    ...
                    {/* Title */}
                    <p className="text-xs font-sans font-medium text-slate-200 group-hover:text-white line-clamp-2 leading-snug">
                      {task.title}
                    </p>
                ...
                {colTasks.length > 15 && (
                  <div className="text-center py-2 text-[10px] font-mono text-slate-400 uppercase tracking-wider">
                    + {colTasks.length - 15} more stories in {col.id}
                  </div>
                )}
              </div>
            </div>
